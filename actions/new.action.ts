...
const getProjectDirectory = (applicationName: Input, directoryOption?: Input): string => {
    return (
...
const generateApplicationFiles = async (args: Input[], options: Input[]) => {
    ...
    const templateName = options.find(
        option => option.name === 'template' && option.value != null,
    )!.value as string;
    const collection: AbstractCollection = CollectionFactory.create(
        (collectionName as Collection) || Collection.NESTCLOUD,
    );
    ...
    await collection.execute(templateName || 'application', schematicOptions);
    console.info();
};
...
const installPackages = async (options: Input[], dryRunMode: boolean, installDirectory: string) => {
...
const printCollective = () => {
    ...
    emptyLine();
    yellow(`Thanks for installing NestCloud ${EMOJIS.PRAY}`);
    ...
    print()(
        `${chalk.bold(`${EMOJIS.WINE}  Donate:`)} ${chalk.underline(
            'https://opencollective.com/nest-cloud',
        )}`,
    );
    emptyLine();
};
